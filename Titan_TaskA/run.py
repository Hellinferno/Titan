...
# Note: When using Ollama (default), API key is not needed
if os.environ.get("USE_OLLAMA", "true").lower() != "true" and not os.environ.get("OPENROUTER_API_KEY"):
    print("⚠️  Warning: OPENROUTER_API_KEY environment variable not set!")
    print("   Set it with: $env:OPENROUTER_API_KEY = 'your-key' (PowerShell)")
    print("   Or: export OPENROUTER_API_KEY='your-key' (Bash)")
    print("   Alternatively, use Ollama: $env:USE_OLLAMA = 'true'")
else:
    if os.environ.get("USE_OLLAMA", "true").lower() == "true":
        print("🦙 Using Ollama (local LLM) for inference")
